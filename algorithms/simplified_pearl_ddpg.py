...
'''
A simplified meta-learning algorithm which assumes that task parameters are an action offset.
Instead of using the latent task encoding as a network input, it is instead subtracted from network actions
to correct for assumed task offset.
This can also load a pretrained no-offset-ddpg agent state as initialization, or even keep
the pretrained no-offset agent frozen.
'''

class SimplifiedPearlDDPGPolicy(Policy):
    def __init__(self, actor_net: MLP, sample_latent_func: Callable, exploration_steps: int, state_size: int, action_size: int):
    ...
    def reset(self, task_index: int) -> None:
        ...
        self.latent = torch.normal(torch.zeros(1, self.action_size), torch.ones(1, self.action_size)).to(DEVICE)
...
class SimplifiedPearlDDPG(Trainer):
    def __init__(self, config: dict, load_dir: Optional[str] = None):
        ...
        self.task_encoder = MLP(
            [obs_size + act_size + 1 + obs_size] + hidden_layers + [2 * act_size]
        ).to(DEVICE)
        
        if load_dir is None:
            if self.algo_config["pretrained_no_offset_ddpg"] is not None:
                # Load state from pretrained agent
                pretrained_dir = self.algo_config["pretrained_no_offset_ddpg"]
                self.critic.load_state_dict(torch.load(os.path.join(pretrained_dir, "critic.pt")))
                self.target_critic.load_state_dict(torch.load(os.path.join(pretrained_dir, "target_critic.pt")))
                self.actor.load_state_dict(torch.load(os.path.join(pretrained_dir, "actor.pt")))
                self.target_actor.load_state_dict(torch.load(os.path.join(pretrained_dir, "target_actor.pt")))
        ...
        self.wrapped_policy = SimplifiedPearlDDPGPolicy(self.actor, self.sample_latent, self.algo_config["exploration_steps"], obs_size, act_size)
    ...
    def sample_latent(self, context_batch: torch.Tensor):
        ...
        Returns:
            torch.Tensor: Mean: shape = (batch, action_size)
            torch.Tensor: Var: shape = (batch, action_size)
            torch.Tensor: Sample: shape = (batch, action_size)
        """
        episode_count, step_count, context_size = context_batch.shape
        act_size = self.config["action_size"]
        
        # Compute task encodings dist for each individual context tuple
        task_encoder_output = self.task_encoder(context_batch.reshape(-1, context_size)).reshape(episode_count, step_count, 2 * act_size)
        latent_mean_per_tuple = task_encoder_output[..., :act_size]
        latent_var_per_tuple = torch.clamp(F.softplus(task_encoder_output[..., act_size:]), min=1e-7)
        ...
        latent = latent_dist.rsample() # shape = (episode_count, action_size)
    ...
    def train_step(self, task_indices: List[int], trajectories: List[List[Trajectory]]) -> dict:
        ...
        for _ in range(self.algo_config["updates_per_train_step"]):
            ...
            # shape = (episode_count * batch_size, action_size)
            latent = latent.unsqueeze(1).expand(episode_count, self.algo_config["batch_size"], act_size).reshape(-1, act_size)
            ...
            # Add the latent action offset onto the observed actions to produce what we think the ground truth environment actions were
            # From here, actor and critic networks update as though there is no offset
            corrected_action = action + latent
            
            # critic loss (q_pred propagates to task encoder)
            with torch.no_grad():
                next_action = self.target_actor(next_state)
                q_target = reward + (1 - done_mask) * self.config["discount_rate"] * self.target_critic(next_state, next_action)   
            q_pred = self.critic(state, corrected_action)
            ...
            if not self.algo_config["freeze_agent"]:
                actor_loss.backward()
            ...
            # Distance between latent_mean and true task action offsets
            true_action_offsets = torch.from_numpy(self.task_params[context_task_indices])
            latent_offset_error = torch.norm(true_action_offsets - latent_mean, dim=1).mean()
            latent_offset_errors.append(latent_offset_error)
        ...
        metrics["latent_offset_error"] = sum(latent_offset_errors) / len(latent_offset_errors)
        return metrics